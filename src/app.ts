...
let slicedDictonary: Array<Array<string>> = [];
let pockets: Array<string> = ["nks","cut","hbl","aro"];
const alphabet = "abcdefghijklmnopqrstuvwxyz";
let trueWords: Array<string> = [];
...
function sliceDictionary(){
  ...
  for(let i = 0; i < big_dictionary.length; i++){
    ...
    let letterBias: number = letterToNumber(firstLetter);
...
function pocketChecker(lastPocket: number, letter: string): number | null{
...
function wordChecker(pocketBias: number, word: string) {
  ...
  for (let k = 1; k < word.length; k++) {
    ...
    if (nextValidPocket == null) return;
...
function dictionarySliceChecker(dictionarySlice: Array<string>, startingPocket: number) {
