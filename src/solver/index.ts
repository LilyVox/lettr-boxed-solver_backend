...
const big_dictionary: Array<string> = fs.readFileSync(wordListPath, 'utf8').split('\n');
const slicedDictionary: Array<Array<string>> = [];
const alphabet = 'abcdefghijklmnopqrstuvwxyz';
let allValidWordsByPocket: {[key:string]:Array<string>} = {};
...
function sliceDictionary() {
  ...
  for (let i = 0; i < big_dictionary.length; i++) {
    ...
    if (filterDuplicateLettersOrShortWords(word)) continue;
    const firstLetter = word[0].toLowerCase();
    const letterBias: number = letterToNumber(firstLetter);
    slicedDictionary[letterBias].push(word);
  }
}
function filterDuplicateLettersOrShortWords(word: string): boolean {
  if (word == undefined || word.length < 3) return true;
  let lastLetter = '';
...
function pocketChecker(pockets: Array<string>, lastPocket: number, letter: string): number | null {
...
function wordChecker(pockets: Array<string>, pocketBias: number, word: string): string | null {
  ...
  for (let k = 1; k < word.length; k++) {
    ...
    if (nextValidPocket == null) return null;
    lastValidPocket = nextValidPocket;
  }
  return word;
}
function dictionarySliceChecker(
  pockets: Array<string>,
  dictionarySlice: Array<string>,
  startingPocket: number
) {
  console.log('checking slice...' + dictionarySlice[0]);
  const trueWordSlice: Array<string> = [];
  for (let j = 0; j < dictionarySlice.length; j++) {
    ...
    if (validWord != null) trueWordSlice.push(word);
  }
  return trueWordSlice;
}

export function wordsFinder(pockets: Array<string>) {
...
export function wordStringer(pockets: Array<string>) {
  ...
  const allStringsFound: {[key:string]:Array<string>} = {};
...
function makeWordString(startingLetter: string, pockets:Array<string>){
  if(allValidWordsByPocket[startingLetter].length == 0) return [];
  const wordString: Array<string>=[];
  ...
  do {
    ...
    if(foundWord.length > 0) wordString.push(foundWord)
    else return wordString;
    
  } while (remainingLetters.length > 0 || wordString.length > 7);
  return wordString;
}
function findWeightedWordByLetter(letter:string, lettersToHit: string){
  // loop over the words in a slice, return the highest one?
  let bestWord:string = "";
  let bestWeight: number = 0;
  if(allValidWordsByPocket[letter].length == 0) return "";
...
function findRemainingLettersToHit(pockets: Array<string>, words:Array<string>){
  const wordsSoFar = words.join("");
  const pocketsJoined = pockets.join("");
  if(wordsSoFar.length == 0) return pocketsJoined;
...
function wordWeight(lettersToHit: string, word: string) {
